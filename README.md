...
### 일회성 명령 실행 (Headless Mode)
대화형 세션을 열지 않고 단일 명령으로 스킬을 실행하는 방식입니다. 
> [!NOTE]
> Headless 모드는 API 직접 호출 방식으로 작동하므로, 대화형 세션 모드에 비해 할당량(Quota Limit) 소모가 더 빠르게 발생할 수 있습니다. 토큰 자원 관리 및 최적화가 필요한 대규모 작업 시에는 가급적 대화형 CLI 모드 사용을 권장합니다.

```bash
gemini -p "/teb:ppt-slides --extracting --output-format md temp/slides/20260408 output/result1.md" -y
```
<br/>
